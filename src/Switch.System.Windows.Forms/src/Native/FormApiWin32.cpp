...
intptr Native::FormApi::Create(System::Windows::Forms::Form& form, bool withClientsize) {
  ...
  case FormStartPosition::WindowsDefaultLocation: bounds = Drawing::Rectangle(CW_USEDEFAULT, CW_USEDEFAULT, withClientsize ? form.ClientSize().Width() : form.Width(), withClientsize ? form.ClientSize().Height() : form.Height()); break;
  }
  ...
  if (withClientsize) {
    ...
    if (form.StartPosition == FormStartPosition::CenterScreen)
      ...
      GetWindowRect(handle, &actualRect);
      bounds = System::Drawing::Rectangle(actualRect.left, actualRect.top, form.Width, form.Height);
    }
    ...
    RECT actualRect = { 0, 0, 0, 0 };
    GetWindowRect(handle, &actualRect);
    form.Location = System::Drawing::Point(actualRect.left, actualRect.top);
    form.Size = System::Drawing::Size(actualRect.right - actualRect.left, actualRect.bottom - actualRect.top);
  }
