...
namespace Examples {
  ...
  public:
    ...
    static void Main() {
      // Creates and initializes a four-dimensional Array of type string.
      auto my4DArray = Array<>::CreateInstance<string, 4>({2, 3, 4, 5});
        ...
        for (int j = my4DArray.GetLowerBound(1); j <= my4DArray.GetUpperBound(1); j++)
          for (int k = my4DArray.GetLowerBound(2); k <= my4DArray.GetUpperBound(2); k++)
            for (int l = my4DArray.GetLowerBound(3); l <= my4DArray.GetUpperBound(3); l++)
              my4DArray.SetValue(Convert::ToString(i) + j + k + l, { i, j, k, l });
